...
func initTest(tc *testCase, ns string, ownerRef *metav1.OwnerReference, t *testing.T) (NSXIPManager, *nsxfake.Clientset) {
	...
	i.Start(ctx.Done())
	return NewNSXVPCIPManager(client, i, ownerRef.Name, ns, tc.podIPPoolType, ownerRef, true, false), client
}
...
func expectedCRLabels(nodeName string, clusterName string) map[string]string {
				...
				core.NewCreateAction(
					...
					&vpcapisv1.IPAddressAllocation{
						ObjectMeta: metav1.ObjectMeta{
							Name:      nodeName,
							Namespace: ns,
							Labels:    expectedCRLabels(nodeName, "test-cluster"),
...
func initTestWithFamilies(tc *testCase, ns string, ownerRef *metav1.OwnerReference, ipv4, ipv6 bool, t *testing.T) (NSXIPManager, *nsxfake.Clientset) {
	...
	_, ctx := ktesting.NewTestContext(t)
	i.Start(ctx.Done())
	return NewNSXVPCIPManager(client, i, ownerRef.Name, ns, tc.podIPPoolType, ownerRef, ipv4, ipv6), client
}
