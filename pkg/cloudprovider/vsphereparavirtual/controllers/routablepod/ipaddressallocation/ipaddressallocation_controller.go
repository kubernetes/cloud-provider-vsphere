...
import (
	...
	nsxinformerv1alpha1 "github.com/vmware-tanzu/nsx-operator/pkg/client/informers/externalversions/nsx.vmware.com/v1alpha1"
...
func NewController(
	...
	nodesLister listerv1.NodeLister,
	nodesSynced cache.InformerSynced,
	ipAddressAllocationInformer nsxinformerv1alpha1.IPAddressAllocationInformer) *Controller {
	logger := klog.FromContext(ctx)

	// Create event broadcaster
	utilruntime.Must(nsxapisv1alpha1.AddToScheme(scheme.Scheme))
...
func (c *Controller) syncHandler(ctx context.Context, key string) error {
	...
	err = c.processIPAddressAllocationCreateOrUpdate(ctx, ipAddressAllocation)
	if err != nil {
		return err
	}
...
func (c *Controller) processIPAddressAllocationCreateOrUpdate(ctx context.Context, ipAddressAllocation *nsxapisv1alpha1.IPAddressAllocation) error {

	var podCIDR string
	for _, condition := range ipAddressAllocation.Status.Conditions {
		if condition.Type == nsxapisv1alpha1.Ready {
	...
	if err := utils.PatchNodeCIDRWithRetry(ctx, c.kubeclientset, node, podCIDR, c.recorder); err != nil {
		return err
	}
