...
type fixture struct {
	...
	ipAddressAllocationsLister []*nsxapisv1alpha1.IPAddressAllocation
...
func newIPAddressAllocation(name string, ipAddressBlockVisibility nsxapisv1alpha1.IPAddressVisibility, cidr string, conditionStatus corev1.ConditionStatus) *nsxapisv1alpha1.IPAddressAllocation {
	return &nsxapisv1alpha1.IPAddressAllocation{
		TypeMeta: metav1.TypeMeta{APIVersion: nsxapisv1alpha1.SchemeGroupVersion.String()},
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
		...
		Status: nsxapisv1alpha1.IPAddressAllocationStatus{
			CIDR: cidr,
			Conditions: []nsxapisv1alpha1.Condition{
				{
					Type:   nsxapisv1alpha1.Ready,
...
func getKey(ipAddressAllocation *nsxapisv1alpha1.IPAddressAllocation, t *testing.T) string {
...
func TestPatchNodeCIDR(t *testing.T) {
	...
	cidr := "192.168.5.0/24"
	f := newFixture(t)
	ipAddressAllocation := newIPAddressAllocation(name, nsxapisv1alpha1.IPAddressVisibilityPrivate, cidr, corev1.ConditionTrue)
...
func TestIPAddressAllocationNotReady(t *testing.T) {
	...
	ipAddressAllocation := newIPAddressAllocation(name, nsxapisv1alpha1.IPAddressVisibilityPrivate, "192.168.5.0/24", corev1.ConditionFalse)
	f.ipAddressAllocationsLister = append(f.ipAddressAllocationsLister, ipAddressAllocation)
	f.objects = append(f.objects, ipAddressAllocation)
...
func TestIPAddressAllocationEmptyCIDR(t *testing.T) {
	...
	ipAddressAllocation := newIPAddressAllocation(name, nsxapisv1alpha1.IPAddressVisibilityPrivate, "", corev1.ConditionTrue)
	f.ipAddressAllocationsLister = append(f.ipAddressAllocationsLister, ipAddressAllocation)
	f.objects = append(f.objects, ipAddressAllocation)
