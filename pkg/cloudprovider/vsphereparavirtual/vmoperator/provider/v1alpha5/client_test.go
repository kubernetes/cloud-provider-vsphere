...
// explicitly. The v1alpha5 Client is read-only for VMs (the CPI never creates
// VMs), so we seed via the underlying fake but must set apiVersion/kind to
// mirror what a real API server stores, consistent with the dynamic client
// convention used in production Create/Update paths.
...
func TestVMSUpdateSetsGVK(t *testing.T) {
	...
	_, err = c.UpdateVirtualMachineService(context.Background(), &vmopv5.VirtualMachineService{
		ObjectMeta: metav1.ObjectMeta{Name: "test-vms", Namespace: testNS},
	})
	assert.NoError(t, err)
	assert.Equal(t, VirtualMachineServiceGVR.Group+"/"+VirtualMachineServiceGVR.Version, capturedAPIVersion,
