...
import (
	...
	fakev5 "k8s.io/cloud-provider-vsphere/pkg/cloudprovider/vsphereparavirtual/vmoperator/adapter/v1alpha5/fake"
...
// seedVM seeds a VirtualMachine into the fake dynamic client with GVK set
// explicitly, matching the convention that production code follows when writing
// objects via the dynamic client (ToUnstructured + explicit apiVersion/kind).
// The v1alpha5 Client is read-only for VMs (the CPI never creates VMs), so
// we seed directly but must set GVK to mirror what a real API server would store.
func seedVM(t *testing.T, fc *dynamicfake.FakeDynamicClient, vm *vmopv5.VirtualMachine) {
	...
	obj["apiVersion"] = providerv5.VirtualMachineGVR.Group + "/" + providerv5.VirtualMachineGVR.Version
			...
			seedVMs: []*vmopv5.VirtualMachine{
				{
					...
					Status:     vmopv5.VirtualMachineStatus{BiosUUID: ""},
				},
			},
			queryUUID: "",
			expectNil: true,
...
func TestAdapter_VirtualMachineServices_Update_Conflict(t *testing.T) {
	...
	// Seed the VirtualMachineService directly via the fake dynamic client
	// (the v1alpha5 provider exposes CreateVirtualMachineService).
	vms := &vmopv5.VirtualMachineService{
		ObjectMeta: metav1.ObjectMeta{Name: "test-vms", Namespace: testNS},
		Spec:       vmopv5.VirtualMachineServiceSpec{Type: vmopv5.VirtualMachineServiceTypeLoadBalancer},
	}
	obj, err := runtime.DefaultUnstructuredConverter.ToUnstructured(vms)
	assert.NoError(t, err)
	obj["apiVersion"] = providerv5.VirtualMachineServiceGVR.Group + "/" + providerv5.VirtualMachineServiceGVR.Version
	obj["kind"] = "VirtualMachineService"
	_, err = fc.Resource(providerv5.VirtualMachineServiceGVR).Namespace(testNS).Create(
		context.Background(), &unstructured.Unstructured{Object: obj}, metav1.CreateOptions{})
	assert.NoError(t, err)
