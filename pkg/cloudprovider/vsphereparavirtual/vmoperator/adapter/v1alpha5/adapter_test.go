...
func seedVM(t *testing.T, fc *dynamicfake.FakeDynamicClient, vm *vmopv5.VirtualMachine) {
	...
	obj["apiVersion"] = clientv5.VirtualMachineGVR.Group + "/" + clientv5.VirtualMachineGVR.Version
	obj["kind"] = "VirtualMachine"
	_, err = fc.Resource(clientv5.VirtualMachineGVR).Namespace(vm.Namespace).Create(
...
func TestAdapter_VirtualMachineServices_Update_Conflict(t *testing.T) {
	...
	c := clientv5.NewWithDynamicClient(fc)
	_, err := c.CreateVirtualMachineService(context.Background(), &vmopv5.VirtualMachineService{
