...
// Package v1alpha2 provides a vmoperator.Interface adapter that translates
// between the hub types and the VM Operator v1alpha2 API.
//
// Hub fields introduced in later API versions are left at their zero values
// and ignored by the business logic when this adapter is active.
package v1alpha2
...
func New(cfg *rest.Config) (*Adapter, error) {
	c, err := vmopclient.NewForConfig(cfg)
	if err != nil {
		return nil, err
	}
...
func NewWithFakeClient(fakeClient *vmopclient.Client) *Adapter {
	return newAdapter(fakeClient)
}

func newAdapter(c *vmopclient.Client) *Adapter {
	return &Adapter{
...
// vmsToInfo converts a v1alpha2 VirtualMachineService to the version-agnostic hub type.
func vmsToInfo(vms *vmopv1.VirtualMachineService) *types.VirtualMachineServiceInfo {
...
// infoToVMS converts a hub VirtualMachineServiceInfo to a v1alpha2 VirtualMachineService.
// Only fields that are set on Create are populated; status is not carried over.
func infoToVMS(info *types.VirtualMachineServiceInfo) *vmopv1.VirtualMachineService {
	...
	for _, p := range info.Spec.Ports {
		vms.Spec.Ports = append(vms.Spec.Ports, vmopv1.VirtualMachineServicePort{
			Name:       p.Name,
			Protocol:   p.Protocol,
			Port:       p.Port,
			TargetPort: p.TargetPort,
...
func applyVMSUpdate(dst *vmopv1.VirtualMachineService, update *types.VirtualMachineServiceInfo) {
