...
import (
	...
	clientfake "k8s.io/cloud-provider-vsphere/pkg/cloudprovider/vsphereparavirtual/vmoperator/client/fake"
...
// bypassing the client layer. VirtualMachine is read-only from the CPI's
// perspective; the write methods were removed from the client.
func seedVM(t *testing.T, fc *dynamicfake.FakeDynamicClient, vm *vmopv1.VirtualMachine) {
	...
	obj["apiVersion"] = vmopclient.VirtualMachineGVR.Group + "/" + vmopclient.VirtualMachineGVR.Version
	obj["kind"] = "VirtualMachine"
	_, err = fc.Resource(vmopclient.VirtualMachineGVR).Namespace(vm.Namespace).Create(
			...
			seedVMs: []*vmopv1.VirtualMachine{
				{
					...
					Status:     vmopv1.VirtualMachineStatus{BiosUUID: ""},
				},
			},
			queryUUID: "",
			expectNil: true,
